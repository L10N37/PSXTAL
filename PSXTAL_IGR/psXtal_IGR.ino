...
// + SCPH1200 REGULAR ANALOGUE CONTROLLER WITH RUMBLE
...
void loop() {
  ...
  if (buffer == 0xF9 || buffer == 0xF2 || buffer == 0xFC) {   //  regular analogue controller error correction (videomode)
    buffer = 0xE5;
    counter++;
    loop();
  }
  else if (buffer == 0xE5 || buffer == 0xD9 || buffer ==  0xF3 || buffer ==  0xDB || buffer ==  0xEB || buffer ==  0xDD) { // ensure the combo has to be held for at least ~5s
    counter++;
    loop();
  }
