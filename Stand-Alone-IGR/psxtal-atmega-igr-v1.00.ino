...
#define shortresetcombo ControllerByteArray[3] == 0xF6 && ControllerByteArray[4] == 0xFC
#define longresetcombo  ControllerByteArray[3] == 0xFE && ControllerByteArray[4] == 0xBC
...
void capturepackets() //Main FOR = bytes, nested FOR = bits
{
    ...
    ControllerByteArray[ControllerBytePosition] = ControllerBytes;  // store the bits into a byte
        ...
        void loop() {
          ...
          if (shortresetcombo && analogueMode||digitalMode) SHORTRST; // SHORT RESET

          if (longresetcombo && analogueMode||digitalMode) LONGRST;  // LONG RESET
